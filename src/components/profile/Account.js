...
function Account(props) {
    ...
    const handleAccountSubmit = async (event) => {
        ...
        if (event.target.profilePic !== undefined && event.target.profilePic.files.length > 0) {
    ...
    const validateFormData = (formData) => {
        const {username, email, password, repeatedPassword, kidAge, secretWord} = formData;
        let validationErros = []
    ...
    <Container component="main" maxWidth="xs">
      ...
      <div className={classes.paper}>
        <Typography component="h1" className={classes.headline} variant="h5">
            ...
            />
            <ParentComponents classes={classes} user={props.user} isVisible={props.user.role === "parent"}></ParentComponents>
            <TextField
                variant="outlined"
                required
                fullWidth
            ...
            {errors.map((error, i) => {
                return (
                    <FormError key={i} text={error}></FormError>
                );
            })}
